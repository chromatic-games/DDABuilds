		...
		<div id="buildTab" class="tab-pane active">
									...
									<div class="panel-body">
										<div class="row">
											...
											foreach ( $towers as $tower ) {
												...
												echo '<div class="col-md-2 defense-placeholder" data-class="'.$tower->fk_class.'">
											        <div class="tower-container defense" data-tower-id="'.$tower->id.'" data-class="'.$tower->fk_class.'"
											        data-du="'.$tower->unitcost.'" data-mana="'.$tower->manacost.'" data-wave="0">
											            <div class="pool">
											                <img class="tower pool" src="'.$tower->getImage().'" title="'.$this->escapeHtml($tower->name).'" />
											            </div>
											            '.$menu.'
											        </div>
											    </div>';
							...
							<div class="panel panel-default">
								...
								<div class="panel-body">
									...
									<div class="form-group">
										...
										<input type="text" placeholder="XP Per Run" class="form-control" id="expPerRun" maxlength="20" />
									</div>
									<div class="form-group">
										<label>Time Per Run:</label>
										<input type="text" placeholder="XP Per Run" class="form-control" id="timePerRun" maxlength="20" />
									</div>
			...
			<div class="container">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<?php
						if ( $this->action === 'add' || $this->build->isCreator() ) {
	...
	$(document).ready(function () {
		...
		for (let el of [$('#requiredStatsHp'), $('#requiredStatsRate'), $('#requiredStatsDamage'), $('#requiredStatsRange')]) {
			el.on('change', function () {
				...
				stats[selectedClass][statsKey] = newValue;
				$(this).val(newValue);
				console.log(stats[selectedClass], stats);
			});
		}

		$('#requiredStatsClass').on('change', function () {
			var selectedClass = $(this).val();
			$('#requiredStatsHp').val(stats[selectedClass].hp);
			$('#requiredStatsRate').val(stats[selectedClass].rate);
			$('#requiredStatsDamage').val(stats[selectedClass].damage);
			$('#requiredStatsRange').val(stats[selectedClass].range);
		});
		...
		$(document)
			...
			.on('contextmenu', '.canvas .tower-container', function (event) {
				$(this).remove();
				calculateDefenseUnits();
				return false;
			});
